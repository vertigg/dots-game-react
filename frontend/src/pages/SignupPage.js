...
class SignupPage extends React.Component {
  ...
  handleSubmit(event) {
    ...
    authService.signup(username, password1)
      .then(() => {
  ...
  render() {
    ...
    return (
      ...
      <div className="form-group">
        ...
        <div className="form-group">
          <label className="form-control-label" htmlFor="password1">Password</label>
          <input
            className={`form-control ${submitted && password1 !== password2 ? 'is-invalid' : ''}`}
            type="password"
            name="password1"
            value={this.state.password1}
            onChange={this.handleChange}
          />
            {(submitted && password1.length <= 5 && password1.length > 0)
            && <p id="passwordHelpInline" className={'text-danger small'}>Password is too short</p>
              }
            {submitted && password1 !== password2
            && <p id="passwordHelpInline" className={'text-danger small'}>Passwords don&apos;t match</p>
              }
        </div>
        <div className="form-group">
